...
public class FeatureGenerator_mod {
    ...
    static {
	features = new String[] { "firstName0", "firstName1" , "firstName2" , "firstName3" , "firstName4","lastName0", "lastName1" , "lastName2" , "lastName3" , "lastName4" }; // add features
	...
	ff.add("theta=1"); // add theta features
	...
	FastVector attributes = new FastVector(12);				// mod capacity to 12
	...
	String lastName = parts[2].toLowerCase(); 					// Add for lastName

	Instance instance = new Instance(features.length + 1); 		// add 1 theta
	instance.setDataset(instances);

	Set<String> feats = new HashSet<String>();
	//////////  add for new features  //////////
	for(int times = 0; times < 5; times++){
		if(firstName.length() > times ){
			feats.add("firstName"+times+"=" + firstName.charAt(times));
		}else
			feats.add("firstName"+times+"=" + " ");
	}
	for(int times = 0; times < 5; times++){
		if(lastName.length() > times ){
			feats.add("lastName"+times+"=" + lastName.charAt(times));
		}else
			feats.add("lastName"+times+"=" + " ");
	}
	feats.add("theta=1");
	//////////  add for new features  //////////
	...
	if (args.length != 2) {
	    System.err
		    .println("Usage: FeatureGenerator_mod input-badges-file features-file");
	    System.exit(-1);
	}
